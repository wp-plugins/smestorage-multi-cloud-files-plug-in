...
if($action=='share'){
	...
	include('home.php?token='.$token.'&message='.$a[1]['statusmessage']);
}elseif($action=='move'){
...
elseif($action=='fax'){
	...
	include('uploader.php?token='.$token.'&action=start&code='.$a[1]['uploadcode'].'&saveornot='.$_REQUEST['saveornot']);
	...
	include('uploader.php?token='.$token.'&action=end');
